...
if (isset($_POST['reg'])) {
    ...
    // Check if the user has already registered for the activity
    $check_sql = "SELECT * FROM `kehadiran` WHERE `idahli`='$user' AND `idaktiviti`='5'";
    $result = mysqli_query($con, $check_sql);

    if (mysqli_num_rows($result) > 0) {
        // User has already registered, show a message
        echo '<script>alert("Anda sudah mendaftar untuk aktiviti ini sebelum ini.") </script>';
    } else {
        // User has not registered, proceed with the registration
        $sql = "INSERT INTO `kehadiran`(`idahli`, `idaktiviti`) VALUES ('$user','5')";
...
<div class="w3-content" style="max-width:1100px;margin-top:80px;margin-bottom:80px">
  ...
  <div class="container">
      <img src="img/per2.jpg" style="width:100%; ">
      </div
    </div>
  </div>
  <!-- Grid -->
  <div class="w3-row w3-container">
    ...
    <div class="info">
      <h3>Nama Aktiviti</h3>
      <p><?php 
        $sql = "SELECT* FROM aktiviti WHERE idaktiviti = '5';";
